...
void Game::Init() {
    ...
    //for (Block& tile : this->Levels[this->Level].Blocks)
...
//            else
//                score += 0;
            this->score = this->Levels[this->Level].score.score;
...
void Game::removeAnimatorOfKilledEnemy(Enemy *enemy) {
    this->Renderer->enemyAnimators.erase(std::remove_if(this->Renderer->enemyAnimators.begin(), this->Renderer->enemyAnimators.end(),
                                              [&](EnemyAnimator *ea) {
                                                  return ea->enemy == enemy;
                                              }));
    //this->Levels[this->Level].removeAnimatorOfKilledEnemy(enemy);

    //startClockEnemyKill = std::clock();
}

void Game::createAnimatorForSpawnedEnemy(Enemy *enemy) {
    //Enemy *enemy = this->Levels[this->Level].createAnimatorForSpawnedEnemy();
    this->Renderer->enemyAnimators.push_back(new EnemyAnimator(enemy, 0.4f, 3.0f, 1.5f, Texture2D()));
}
...
void Game::update(GameLevel *s) {
    ...
    for (EnemyAnimator *ea : this->Renderer->enemyAnimators) {
        if (std::find(this->Levels[this->Level].Enemies.begin(), this->Levels[this->Level].Enemies.end(), ea->enemy) != this->Levels[this->Level].Enemies.end()) {
            continue;
        } else {
            enemy = ea->enemy;
            break;
...
void Game::initStates() {
    this->Levels[this->Level].score.score = this->score;

    for (Block &b : this->Levels[this->Level].Blocks) {
