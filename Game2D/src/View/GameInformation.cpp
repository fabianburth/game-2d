	...
	:position{ position }, amountDisplayBlocks{ static_cast<unsigned int>(amountDisplayBlocks) }, size{ size }
{
	...
	{
		displayBlocks.emplace_back(new DisplayBlock({ position[0] - static_cast<float>(i) * (0.5f * Constants::WIDTH_UNIT), position[1] }, size, ResourceManager::GetTexture("placeholder")));
	}
}
